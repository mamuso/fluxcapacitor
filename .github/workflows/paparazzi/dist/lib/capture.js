    ...
    this.downloadCurrent = () =>
      __awaiter(this, void 0, void 0, function*() {
        if (this.current && this.current.captures) {
          yield this.current.captures.forEach(capture =>
            __awaiter(this, void 0, void 0, function*() {
        ...
        try {
          ...
          for (; i < iMax; i++) {
            ...
            for (; j < jMax; j++) {
              ...
              const filename = `${slugify_1.default(page.id)}.${
                this.config.format
              }`
              ...
              /** Upload images */
              capture.url = yield this.store.uploadfile(
                `${this.config.date}/${device.id}/${filename}`,
                localfilepath
              )
              /** Write capture in the DB */
              yield this.db.createCapture(
                this.dbReport,
                this.dbDevice,
                dbpage,
                capture
        ...
        try {
          ...
          for (; i < iMax; i++) {
            ...
            for (; j < jMax; j++) {
              ...
              const dbpage = yield this.db.createPage(page, this.dbReport)
              const capture = yield this.db.getCapture(
                this.dbReport,
                this.dbDevice,
                dbpage
              )
              ...
              capture.urlmin = yield this.store.uploadfile(
        ...
        try {
          ...
          for (; i < iMax; i++) {
            ...
            for (; j < jMax; j++) {
              ...
              const filenamediff = `${slugify_1.default(page.id)}-diff.${
                this.config.format
              }`
              ...
              const dbpage = yield this.db.createPage(page, this.dbReport)
              let capture = yield this.db.getCapture(
                this.dbReport,
                this.dbDevice,
                dbpage
              )
              /** Compare */
              diff = yield this.compare.compare(
                localfilepath,
                currentfilepath,
                localfilepathdiff
              )
              ...
              if (diff && diff > 0) {
                capture.urldiff = yield this.store.uploadfile(
